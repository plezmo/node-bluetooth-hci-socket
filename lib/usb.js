...
BluetoothHciSocket.prototype.bindUser = function(devId) {
  ...
  if (process.env.BLUETOOTH_HCI_SOCKET_USB_VID && process.env.BLUETOOTH_HCI_SOCKET_USB_PID) {
    ...
    this._usbDevice = usb.findByIds(0x0a5c, 0x21ec) || usb.findByIds(0x0a5c, 0x21e8) || usb.findByIds(0x19ff, 0x0239) || usb.findByIds(0x0a12, 0x0001) || usb.findByIds(0x0b05, 0x17cb) || usb.findByIds(0x8087, 0x07dc);
  }
  ...
  this._usbDevice.on('error', this.onHciError.bind(this));
};
...
BluetoothHciSocket.prototype.start = function() {
  if (this._mode === 'raw' || this._mode === 'user') {
    ...
    this._hciEventEndpoint.on('error', this.onHciError.bind(this));
    ...
    this._aclDataOutEndpoint.on('error', this.onHciError.bind(this));
  }
};
...
BluetoothHciSocket.prototype.write = function(data) {
  ...
  if (this._mode === 'raw' || this._mode === 'user') {
    ...
    if (HCI_COMMAND_PKT === type) {
      this._usbDevice.controlTransfer(usb.LIBUSB_REQUEST_TYPE_CLASS | usb.LIBUSB_RECIPIENT_INTERFACE, 0, 0, 0, data.slice(1),  function(err) {
        if(err) {
...
BluetoothHciSocket.prototype.onHciError = function(error) {
  console.error("bluetooth-hci-socket - receieved error " + error + " on " + this._usbDevice.deviceAddress);
